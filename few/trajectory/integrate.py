...
from ..utils.utility import check_for_file_download, get_ode_function_options, get_default_orbital_resonance_model_for_function, Y_to_xI, ELQ_to_pex, get_fundamental_frequencies, get_kerr_geo_constants_of_motion, get_separatrix_interpolant
...
class Integrate:
    def __init__(
        ...
        orbital_resonances = False,
        which_resonances = "all",
    ):
        ...
        if not orbital_resonances:
            self.orbital_resonances = False
            self.all_resonances = []
            self.resonances_remaining = []
            self.resonance_crossing_times = {}

        elif orbital_resonances == "default" or orbital_resonances:
            self.orbital_resonances = True
            self.resonance_function_dict = get_default_orbital_resonance_model_for_function(func[0], which_resonances)

            self.all_resonances = list(self.resonance_function_dict.keys())  # descending order (1 -> 0)
            self.resonances_remaining = self.all_resonances.copy()
            self.resonance_crossing_times = {}

        else:
            raise ValueError("kwarg `orbital_resonances` must be either False, 'default' or True.")
            
    def next_resonance(self):
        if len(self.resonances_remaining) > 0:
            return self.resonances_remaining[0]     
        else:
            return None
    
    def update_next_resonance(self):
        self.resonances_remaining.pop(0)

    def reset_resonance_tracker(self, y_start):
        self.resonances_remaining = self.all_resonances.copy()
        self.resonance_crossing_times = {}

        if self.resonances_remaining is not None:
            p, e, x = y_start[:3]
            if self.convert_Y:
                x = Y_to_xI(self.a, p, e, x)
            frequencies = get_fundamental_frequencies(self.a, p, e, x)
            freq_ratio = np.sign(frequencies[1])*frequencies[2]/frequencies[1]
            for resonance_inds in self.all_resonances:
                if freq_ratio - resonance_inds[0]/resonance_inds[1] < 0:
                    self.update_next_resonance()
    ...
    def integrate(self, t0: float, y0: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        t = t0
        h = self.dt_dimensionless
        self.reset_resonance_tracker(y0)  # resets the resonance tracker and removes resonances we've already passed
        ...
        while t < self.tmax_dimensionless:
            ...
            if status == 9:
                ...
                # y_old = y.copy()
                action = self.action_function(t, y, t_prev, y_prev, h)  # TODO: remove h once resonances fixed
                # print(y - y_old)
            if action == "stop":
...
class APEXIntegrate(Integrate):
    ...
    def resonance_inner_func(
            self, h, t, y_out, resonance_ratio
    ):  
        self.y_temp = y_out.copy()
        status, self.t_final_temp, self.h_final_temp = self.dense_integrator.take_step(t, h, self.y_temp, self.tmax_dimensionless)
        p, e, x = self.y_temp[:3]
        if self.convert_Y:
            x = Y_to_xI(self.a, p, e, x)
        frequencies = get_fundamental_frequencies(self.a, p, e, x)
        return np.sign(frequencies[1])*frequencies[2]/frequencies[1] - resonance_ratio # we want this to go to zero

    def step_to_resonance_boundary(
            self, t, y, resonance_ratio, h_attempted
    ):  
        y_out = y.copy()

        ##### Once we have fdot easily: compute this
            # frequencies = get_fundamental_frequencies(self.a, y_out[0], y_out[1], y_out[2])
            # fdots = <magic function!>
            # delta_omtheta = (frequencies[1] - frequencies[2]/resonance_ratio)
            # step_size = delta_omtheta * (resonance_ratio**2/fdots[2]**2 + 1/fdots[1]**2)**0.5
            # result = brentq(self.resonance_inner_func, step_size / 2, step_size * 2, args=(t, y_out, resonance_ratio),maxiter=MAX_ITER, xtol=INNER_THRESHOLD, rtol=1e-8, full_output=True)
        #####
        # For now: quick and dirty, solve the entire interval. TODO fix this!
        result = brentq(self.resonance_inner_func, 0, h_attempted, args=(t, y_out, resonance_ratio),maxiter=MAX_ITER, xtol=INNER_THRESHOLD, rtol=1e-8, full_output=True)
        # the above modifies some internal thing y_temp

        if result[1].converged:
            return result[0], self.y_temp
        else:
            raise RuntimeError(f"Resonance root-finding operation for resonance ratio {resonance_ratio:.2f} did not converge within MAX_ITER.")

    def perform_resonance_crossing(
            self, t_prev, y_prev, resonance_inds, h_attempted
    ):
        # step to the resonance, apply the jump and return the new point
        resonance_ratio = resonance_inds[0]/resonance_inds[1]

        # first: perform the step from y_res to the resonance
        step_res, y_res = self.step_to_resonance_boundary(t_prev, y_prev, resonance_ratio, h_attempted)
        t_res = t_prev + step_res  # the time at which the resonance occurs

        print(f"The {resonance_inds} resonance occured at time {t_res*self.Msec:.5e}, p {y_res[0]:.2f}")

        self.save_point(t_res * self.Msec - 1e-15, y_res)  # slightly offset, for later
        self.resonance_crossing_times[resonance_inds] = t_res * self.Msec

        # incr = np.array([
        #     1e-3, -2e-4, 1e-5, 0., 0., 0.,
        # ])

        res_dict_here = self.resonance_function_dict[resonance_inds]
        svals = np.array(list(res_dict_here.keys()))
        incr = 0

        p, e, Y = y_res[:3]
        if self.convert_Y:
            x = Y_to_xI(self.a, p, e, Y)

        # REMEMBER TO DIVIDE THE CURRENT PHASE BY THE MASS RATIO IN THE FUNCTION AS WE ARE INTEGRATING ON RR TIMESCALE
        Phi_res = (y_res[4]*resonance_inds[0] + y_res[5]*resonance_inds[1])/self.epsilon

        for sth in svals:  # TODO: replace this loop with a loop per variable over an array operation (ish)  - or better, shift everything to C....
            G_s_res = res_dict_here[sth](self.a, e, x)   # evaluate spline
            sgn = sth  #  add sign of frequency
            Tres = 1.  # add Tres
            incr += Tres * G_s_res * np.cos(np.pi/4 + sth*Phi_res)

        y_res[:3] -= incr * 1e5  # TODO per variable (see above)
        # print(y_res, y)
        return t_res, y_res

    def action_function(
        self, t: float, y: np.ndarray, t_prev: float, y_prev: np.ndarray, h_attempted: float
        ...
        if self.orbital_resonances:
            if self.bool_integrate_backwards:
                raise ValueError("Resonances and backwards int not yet supported")

            # Resonances: ignore backward integration for now
            resonance_inds = self.next_resonance()   # TODO gives the resonance index (from a property function that checks the next item on the list)
            if resonance_inds is None:
                return t, h_attempted # we've crossed all of the resonances we were looking for
            resonance_ratio = resonance_inds[0]/resonance_inds[1]

            # check if we crossed the boundary of this resonance
            e, x = y[1:3]
            if self.convert_Y:
                x = Y_to_xI(self.a, p, e, x)
            frequencies = get_fundamental_frequencies(self.a, p, e, x)
            ratio_difference = np.sign(frequencies[1])*frequencies[2]/frequencies[1] - resonance_ratio   # k Omega_theta = n Omega_r  e.g. 2 Omega_theta = 3 Omega_r

            if ratio_difference < 0:# check if the sign of this difference is negative (i.e. did we cross the resonance)
                t_new, y_new = self.perform_resonance_crossing(t_prev, y_prev, resonance_inds, h_attempted)  # TODO, use sigmoid and place four points at (t_res - Tres/2, t_res - Tres/4, t_res, t_res + Tres/4) then return y(t + Tres/2)
                y[:] = y_new[:]    
                self.reset_solver()   # reset to remove cached derivatives
                self.update_next_resonance()  # increment internal counter to next resonance
                return t_new, h_attempted
            else:
                return t, h_attempted
    ...
    def inner_func(self, h, t, y, y_out):
        ...
        # TODO once we can build the interpolant, we can just evaluate it at step size h rather than integrating!
        try:
